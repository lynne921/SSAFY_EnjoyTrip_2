...
서울, 대구, 부산... 도시 클릭했을 때
		document.getElementById("search-area").addEventListener("click", function () {
		    ...
		    if (areaCode !== '0') {
		    	fetch("/EnjoyTrip_BackEnd_seoul13-team2/plan?action=areaarea&area="+areaCode)//db에서 구군 데이터 가져와서
		...
		document.getElementById("btn-search").addEventListener("click", function () {
		    ...
		    fetch("/EnjoyTrip_BackEnd_seoul13-team2/plan?action=search&area="+areaCode+"&areaarea="+areaareaCode+"&type="+contentTypeId)//db에서 검색 조건에 맞는 데이터 가져와서
		        .then((response) => response.json())
		        .then((data) => {
		        console.log(data);
		        makeTripDot(data)
